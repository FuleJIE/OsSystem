    ...
    <div class="notification-menu">
      ...
      <ul>
        <li v-for="(item, key) in menuList" :key="key">
          <button
            v-if="(collapse && !item.collapse) || !collapse"
            @click="toggleActiveClass(key, item.title)"
            :class="isActive.includes(key) && 'active'"
          >
            <i
              :class="
                item.icon === 'bluetooth'
...
export default {
  ...
  data() {
    return {
      ...
      collapse: false,
      selectedRangeWidth: getLocal("windows-settings").brightness
        ? 100 - Math.abs(getLocal("windows-settings").brightness)
        : 100,
      isActive: [],
      menuList: [
        {
  ...
  watch: {
    brightness() {
      document.getElementById("brightness-overlay").style.opacity = Math.abs(
        this.brightness / 100
      );
  ...
  created() {
    if (getLocal("windows-settings").nightLight) {
      this.menuList.forEach(
        (item, key) => item.title === "Night Light" && this.isActive.push(key)
      );
    }
  },
  mounted() {
    if (getLocal("windows-settings").brightness) {
      document.getElementById("brightness-overlay").style.opacity = Math.abs(
        getLocal("windows-settings").brightness / 100
      );
    }
  },
};
